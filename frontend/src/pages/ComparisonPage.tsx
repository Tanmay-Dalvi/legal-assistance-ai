    ...
    <div className="mt-3 grid gap-3 text-xs sm:grid-cols-2">
      {[['Document A', change.evidence_a], ['Document B', change.evidence_b]].map(([label, references]) => (
        <div key={label as string}>
          {(references as ComparisonChange['evidence_a']).map((reference, index) => <EvidenceCard key={index} evidence={reference} documentLabel={label as string} />)}
        </div>
      ))}
    </div>
  )
...
export default function ComparisonPage() {
  ...
  return <main className="workspace-page">
    ...
    {!loading && documents.length === 0 && <div className="mt-6 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center text-slate-600">Upload two ready documents from the Dashboard to compare them.</div>}
    {loading && <div className="mt-6 flex items-center gap-3 rounded-lg bg-slate-50 p-5" role="status"><LoaderCircle className="h-5 w-5 animate-spin" aria-hidden="true" />Comparing the selected documents. This may take a moment.</div>}
